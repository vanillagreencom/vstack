...
Never spawn a fresh agent when an existing one of the same type is alive — message it instead. Before creating agent tasks, check the task list for PENDING tasks with the same prefix. If an agent appears stuck (no progress across multiple idle cycles): shut down → respawn → re-create tasks → re-delegate.
...
After delegation, wait for the agent's return message. Idle notifications are normal. On idle notification, check the task list:
...
The orchestrator never edits or writes code in the worktree. Always delegate to the domain agent. If stuck, follow re-delegation protocol: shut down → respawn → re-create tasks → re-delegate. The orchestrator may run read-only commands and invoke scripts.
