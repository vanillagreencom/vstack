...
mod tests {
    ...
    fn config_adds_explicit_agent_skills() {
        ...
        config
            .agent_skills
            .insert("iced".into(), vec!["iced-rs".into(), "visual-qa".into()]);
        let available = vec!["iced-rs".into(), "visual-qa".into(), "other".into()];
        let matched = config.skills_for_agent("iced", &AgentRole::Engineer, &available);
        assert!(matched.contains(&"iced-rs".to_string()));
        assert!(matched.contains(&"visual-qa".to_string()));
    ...
    fn reviewer_agent_checks_stripped_prefix() {
        ...
        config
            .agent_skills
            .insert("iced".into(), vec!["visual-qa".into()]);
        let available = vec!["iced-rs".into(), "visual-qa".into()];
